...
with st.sidebar:
    st.header("監控參數")
    st.write(f"監控檔數: {len(stock_list)}")
...
# =============================
st.subheader("⏱ 系統時間")
st.markdown(f"<span id='time'>{now_taipei().strftime('%H:%M:%S')}</span>", unsafe_allow_html=True)
st_autorefresh = st.experimental_rerun  # Streamlit 自動刷新
...
st.markdown(f"**最後更新時間:** {st.session_state.last_update}")

# =============================
# Streamlit 自動刷新每秒
# =============================
st_autorefresh(interval=1000, limit=None, key="time_autorefresh")
