...
def run_scan_logic(stock_codes, status_placeholder=None):
    ...
    for i in range(0,total,batch_size):
        ...
        if status_placeholder: status_placeholder.info(f"⏳ 掃描批次 {i//batch_size+1}/{(total//batch_size)+1}")
        try:
            ...
            for code in batch:
                try:
                    ...
                    res_type=""
                    mv=list(ma.values())
                    if max(mv)/min(mv)<1.06 and price>ma[5]>ma[10]>ma[20]>ma[60]>ma[100]>ma[200] and d['ma200_b']<0.08: res_type="六線多排"
                    elif max(mv[:-1])/min(mv[:-1])<1.06 and price>ma[5]>ma[10]>ma[20]>ma[60]>ma[100] and d['ma100_b']<0.08: res_type="五線多排"
                    elif max(mv[:4])/min(mv[:4])<1.06 and price>ma[5]>ma[10]>ma[20]>ma[60] and d['ma60_b']<0.08: res_type="四線多排"
                    elif max(mv[:3])/min(mv[:3])<1.06 and price>ma[5]>ma[10]>ma[20] and d['ma20_b']<0.08: res_type="三線多排"
                    elif max(mv[:2])/min(mv[:2])<1.06 and price>ma[5]>ma[10] and d['ma10_b']<0.15: res_type="二線多排"
                    if res_type:
                        all_found.append({"股票代號":code,"價格":price,"漲幅%":RK_p,
                                          "成交量":vol,"型態":res_type,"更新時間":now_taipei().strftime("%Y-%m-%d %H:%M:%S")})
                except: continue
        except: continue
...
df_local,last_upd=load_cache()
st.session_state.df_results = df_local if not df_local.empty else st.session_state.df_results
...
if curr_min in SCHEDULE_TIMES and st.session_state.last_run_min != curr_min:
    ...
    if sync_success and is_recent:
        status_placeholder.success(f"✅ 偵測到雲端已有最新數據 ({curr_min})，已同步。")
        time.sleep(2)
        st.rerun()
    else:
        status_placeholder.warning("☁️ 雲端無最新數據，開始本地掃描...")
        ...
        status_placeholder.success(f"✅ {curr_min} 掃描完成並同步！")
        time.sleep(2)
        st.rerun()
...
with st.sidebar:
    st.header("⚙️ 數據管理")
    st.info(f"監控股票檔數: {len(stock_list)}")
    
    if st.button("🔄 同步雲端快取"):
        with st.spinner("同步中..."):
            if sync_from_cloud():
                st.toast("✅ 已同步 GitHub 最新資訊")
                time.sleep(1)
                st.rerun()
            else: st.error("同步失敗")
...
if os.path.exists(HISTORY_PATH):
    ...
    if not df_history.empty:
        st.dataframe(df_history.sort_values(by="更新時間",ascending=False),use_container_width=True)
    else:
        st.info("⌛ 歷史訊號暫無資料")
else:
    st.info("⌛ 歷史訊號暫無資料")
