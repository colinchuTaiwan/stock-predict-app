...
# 1. 頁面配置與 GitHub 雲端設定
# =============================
st.set_page_config(page_title="台股多頭排列融合掃描器", layout="wide")
...
def upload_to_github(content_dict):
    ...
    try:
        ...
        data = {"message": f"update {now_taipei()}", "content": content_b64, "branch": "main"}
        if sha: data["sha"] = sha
...
def run_scan_logic(stock_codes, status_placeholder):
    ...
    for i in range(0, total, batch_size):
        ...
        status_placeholder.info(f"⏳ 正在掃描批次 {i//batch_size+1}/{(total//batch_size)+1}...")
        try:
...
if curr_min in SCHEDULE_TIMES and st.session_state.last_run_min != curr_min:
    ...
    status_msg = st.empty()
    new_results = run_scan_logic(stock_list, status_msg)
